...
AC_DEFUN([TEST_NET],[
  ...
  dnl AS_IF([test "x$with_net" = "xyes"], [
    ...
    dnl NOTIFY([getifaddrs])
    dnl AC_COMPILE_IFELSE([
      dnl AC_LANG_SOURCE([[
        dnl #include <ifaddrs.h>
        dnl int main(void) {
          dnl struct ifaddrs *ifaddr;
          dnl if (-1 == getifaddrs(&ifaddr)) {
            dnl return 0;
          dnl }
          dnl freeifaddrs(ifaddr);
          dnl return 0;
        dnl }
      dnl ]])
    dnl ],[],[
      dnl COMPILE_FAILED([getifaddrs])
      dnl ]
    dnl )
