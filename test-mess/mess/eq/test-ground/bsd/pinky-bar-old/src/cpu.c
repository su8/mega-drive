...
get_cpu(char *str1) {
  ...
  percent        = CLOCKZ * (diff_total - diff_idle) / diff_total;
...
get_cpu_temp(char *str1) {
  ...
  if (9999 < temp) { // > 99C
	FILL_UINT_ARR(str1, temp/1000);
  } else {
    FILL_UINT_ARR(str1, ((999 < temp) ?
